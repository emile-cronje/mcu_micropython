...
class Cache:
    ...
    def write_to_device(self, blocks: list[Block]) -> None:
        ...
        spi.write(b"\xff")
        if len(blocks) == 1:
            # Single-block write (CMD24) using existing helper which toggles CS.
            if cmd(24, blocks[0].block_num * cdv, 0) != 0:
                raise OSError(5)
            write(_TOKEN_DATA, blocks[0].content)
        else:
            # Multi-block write (CMD25) keeping CS asserted for the whole transfer.
            if cmd(25, blocks[0].block_num * cdv, 0, release=False) != 0:
                # release the card and report error
                cs(1); spi.write(b"\xff")
                raise OSError(5)

            # Stream all blocks contiguously without toggling CS.
            for block in blocks:
                # send: data token, payload, dummy CRC
                spi.read(1, _TOKEN_CMD25)
                spi.write(block.content)
                spi.write(b"\xff")
                spi.write(b"\xff")
                # check data response token (0bXXX0_0105 means "data accepted")
                if (spi.read(1, 0xFF)[0] & 0x1F) != 0x05:
                    cs(1); spi.write(b"\xff")
                    raise OSError(5)
                # busy wait while card programs the block
                while spi.read(1, 0xFF)[0] == 0x00:
                    pass

            # send STOP_TRAN token and wait until not busy
            spi.read(1, _TOKEN_STOP_TRAN)
            spi.write(b"\xff")
            while spi.read(1, 0xFF)[0] == 0x00:
                pass

            # finally release CS and provide trailing clocks
            cs(1)
            spi.write(b"\xff")
    ...
    def __init__(
        ...
        **debug_flags,
        ...
        if self._debug_flags.get("analytics", False):
            ...
            # fmt: off
            print("Analytics not available. To use Analytics.log or Analytics.collect etc. you need to have the analytics.py file in the import path. File available inside repo lib folder. A mock class that does nothing will be used")  # fmt: skip
            class Analytics:
