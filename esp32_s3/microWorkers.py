...
class MicroWorkers :
    ...
    def _workerThreadFunc(self, arg) :
        ...
        self.log('MicroWorkers: Thread #%s created' % threadID)
        
        while True :
            self._workersLock.acquire()
            
            try :
                jobName, jobFunc, jobArg, jobCBFunc = self._jobs.pop()
            except :
                jobFunc = None
                self.log('MicroWorkers: Thread #%s waiting for a job...' % threadID)
                self._workersLock.acquire()
            ...
            if jobFunc :
                ...
                self.log('MicroWorkers: Job %s starts in thread #%s\r\n' % (jobName, threadID))
                
                try :
                    jobResult = jobFunc(jobName, jobArg)
                    self.log('MicroWorkers: Job %s finished successfully in thread #%s' % (jobName, threadID))
                except Exception as ex :
                    jobResult = None
                    self.log('MicroWorkers: Job %s raises an exception in thread #%s : %s' % (jobName, threadID, ex))
                    
                if jobCBFunc :
                    try :
                        jobCBFunc(jobName, jobArg, jobResult)
                    except Exception as ex :
                        self.log('MicroWorkers: Finished callback of job %s raises an exception in thread #%s : %s' % (jobName, threadID, ex))
    ...
    def AddJob(self, name, function, arg=None, onFinished=None) :
        if function:
            ...
            self.log('MicroWorkers: Job %s added' % name)
            try :
                self._workersLock.release()
            except :
                pass
