...
class MQTT_base:
    ...
    def __init__(self, config):
        ...
        self._sock = None
        self._sta_if = WANConnect()
        self._sta_if.connect(self._ssid, self._wifi_pw)
        ...
        if ESP8266:
            ...
            for _ in range(60):  # Break out on fail or success. Check once per sec.
                ...
                if PYBOARD and (s.status() not in (1, 2)):
                    break
                elif ESP32 and s.isconnected():#s.status() != network.STAT_CONNECTING:
                    break
