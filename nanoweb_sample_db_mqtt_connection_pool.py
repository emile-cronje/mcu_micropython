...
import uhashlib
from WANConnect import WANConnect
...
async def main():
    ...
    cpuMon = CPUMon()
    useCore1 = True
    loop.create_task(cpuMon.main(worker_func = clockTest, useCore1 = useCore1, showUsage = True))
    
#    asyncio.create_task(monitorStatusQueues(_success_q, _error_q))
    #config['subs_cb'] = callback
    #config['con	nect_coro'] = conn_han
    #MQTTClient.DEBUG = True            
    
#    for brokerId in mqttBrokers:
 #       config['server'] = brokerId
  #      mqtt_client = MQTTClient(config)
   #     loop.create_task(mqtt_run(mqtt_client))        
...
if (useMem == False):
    if (useRAMDisk == True):
        ...
        os.mkdir(dir)
    elif ((useSDDisk == True) & (useSoftSPI == False)):
        print("HardSPI...")        
        rootDir = '/sd'
        sdDir = rootDir + '/btree_storage'            
        ...
        #sdDir = '/sd/sqlite'
        #os.mkdir(sdDir)
    elif ((useSDDisk == True) & (useSoftSPI == True)):
        print("SoftSPI...")
